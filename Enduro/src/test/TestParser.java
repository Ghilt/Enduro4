...
public class TestParser {
	...
	public void testParseSimple() throws ParserException {
		...
		Map<Integer, Competitor> competitors = parser.parse(input);
		assertEquals(competitors.get(1).getStartTimes().get(0), new Time(
				"12.11.10"));
		assertEquals(competitors.get(2).getStartTimes().get(0), new Time(
				"05.11.19"));
	}
	...
	public void testParseComplex() throws ParserException {
		...
		assertEquals(competitors.get(1).getStartTimes().get(0), new Time(
				"12.11.10"));
		assertEquals(competitors.get(1).getFinishTimes().get(0), new Time(
				"18.11.10"));

		assertEquals(competitors.get(2).getStartTimes().get(0), new Time(
				"05.11.19"));
		assertEquals(competitors.get(2).getFinishTimes().get(0), new Time(
				"20.11.19"));
	}
	...
	public void testMultipleFinishTimes() throws ParserException {
		...
		assertEquals(competitors.get(1).getFinishTimes().get(0), new Time(
				"12.11.10"));
		assertEquals(competitors.get(1).getFinishTimes().get(1), new Time(
				"13.21.10"));

		assertEquals(competitors.get(2).getFinishTimes().size(), 2);
		assertEquals(competitors.get(2).getFinishTimes().get(0), new Time(
				"05.11.19"));
		assertEquals(competitors.get(2).getFinishTimes().get(1), new Time(
				"14.33.10"));

		assertEquals(competitors.get(3).getFinishTimes().size(), 1);
		assertEquals(competitors.get(3).getFinishTimes().get(0), new Time(
				"14.11.10"));
